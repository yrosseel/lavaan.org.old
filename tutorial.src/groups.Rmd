...
to the argument in the and function calls. By default, the same model is fitted
in all groups. In the following example, we fit the H&S CFA model for the two
...
vector of arguments, one for each group. If you use a single element instead of
a vector, that element will be applied for all groups (note: this is NOT true
for labels, since this would imply equality constraints). For example:
...
of the indicator to be equal across (two) groups, you can write:
...
in the `cfa` or `sem` function calls.  For example, to constrain (all) the
...
loadings, the following keywords are currently supported:
...
If you omit the arguments, all parameters are freely estimated in each group
(but the model structure is the same).
