...
fixed to zero by default, while the latent variable intercepts/means are freely
estimated. A slightly more complex model adds two regressors (`x1` and `x2`)
that influence the latent growth factors. In addition, a time-varying covariate
that influences the outcome measure at the four time points has been added to
the model. A graphical representation of this model is presented below.

```{r growth, echo=FALSE, message=FALSE, dev=c("png","pdf"), dpi=c(140,100), fig.cap="A growth curve examples"}
library(semPlot)
...
'
fit <- growth(model, data=Demo.growth)
semPaths(fit, layout="tree", rotation=3,
exoVar=FALSE,
exoCov=FALSE,
intercepts=FALSE,
